...
public class ShardResultSet extends ShardResultSetAdaptor implements ResultSet {
	...
	public void addResultSet(List<ResultSet> rss) {
		this.actualResultSets.addAll(rss);
	}
	...
	public void init(List<List<RowData>> firstResult, List<List<RowData>> secondResult, MergeContext context,
			int offset, int splitOffset, int splitNum, int limit, List<Long> countResult) throws SQLException {
		checkClosed();
		if (!dataInited) {
			dataMerger.merge(this, firstResult, secondResult, context, offset, splitOffset, splitNum, limit,
					countResult);
		}
		dataInited = true;
	}
