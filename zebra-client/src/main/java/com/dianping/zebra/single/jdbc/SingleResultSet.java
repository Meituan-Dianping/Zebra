...
public class SingleResultSet implements ResultSet {
	...
	public SingleResultSet(ResultSet resultSet, List<JdbcFilter> filters) {
				...
				JdbcFilter chain = new DefaultJdbcFilterChain(filters) {
					...
					public void closeSingleResultSet(SingleResultSet source, JdbcFilter chain) throws SQLException {
						if (index < filters.size()) {
							filters.get(index++).closeSingleResultSet(source, chain);
						} else {
							source.closeOrigin();
						}
					}
				};
				chain.closeSingleResultSet(this, chain);
			} else {
				closeOrigin();
			}
		}
