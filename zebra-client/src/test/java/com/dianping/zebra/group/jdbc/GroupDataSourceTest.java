...
public class GroupDataSourceTest extends MultiDatabaseTestCase {
	...
	public void test_build_group_datasource_config_with_refresh() {
		...
		Assert.assertEquals(true,
				checkConfig(ds.getConfig().getDataSourceConfigs().get("db-n1-write").getProperties(), "initialPoolSize", "2"));
	}
	...
	//	@Test
	public void testIdcAware() throws Exception {
		...
		for(int i = 0; i < 100; ++i) {
			try {
				...
				System.out.println("ID\tC1\tC2\tN1\tTime");
				while (r.next()) {
					System.out.println(r.getInt(1) + "\t" + r.getString(2) + "\t" + r.getString(3) + "\t" + r.getInt(4) + "\t" + r.getTimestamp(5));
				}
				r.close();
				Thread.sleep(2000);
			} catch (Exception e) {
