	...
	public void test() {
		...
		try {
			...
			for (int i = 0; i < 5; i++) {
				...
				tasks.add(new PreparedStatementExecuteUpdateCallable(stmts, SqlAliasManager.getSqlAlias()));
			}

			List<Future<UpdateResult>> futures = SQLThreadPoolExecutor.getInstance(false).invokeSQLs(tasks);
